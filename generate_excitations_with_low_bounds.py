# ----------------------------------------------------------------------------------------------------------------------
# The goal of this script is to find optimal excitations for different levels of co-contraction when tracking
# joint angle. We set different lower bounds for antagonist muscle corresponding co-contraction level. This will
# over-activate agonist muscle in physiological way.
# The next step (generate_final_data_to_track.py) will be to track the agonist muscles and the joints angles to obtain
# more physiological data for antagonist muscles.
# ----------------------------------------------------------------------------------------------------------------------
import biorbd
...
if __name__ == "__main__":
    ...
    for co in range(len(excitations_init)):
        ...
        objectives.add(Objective.Lagrange.MINIMIZE_STATE, weight=1000, states_idx=np.array(range(nbQ, nbQ * 2)))
        ...
        if plot:
            ...
            plt.plot(q_ref.T)
            plt.legend(labels=["Q estimate", "Q truth"], bbox_to_anchor=(1, 1), loc="upper left", borderaxespad=0.0)
            plt.subplot(212)
            plt.plot(X_est[biorbd_model.nbQ() : biorbd_model.nbQ() * 2, :].T, "x")
            plt.gca().set_prop_cycle(None)
            plt.plot(dq_ref.T)
            plt.legend(
                labels=["Qdot estimate", "Qdot truth"], bbox_to_anchor=(1, 1), loc="upper left", borderaxespad=0.0
            )
            ...
            plt.legend(labels=["u_ref", "u_est"], bbox_to_anchor=(1.05, 1), loc="upper left", borderaxespad=0.0)
            plt.show()

        # Save data if flag is on True
        if save_data:
            if co == 0:
                ocp.save_get_data(sol, f"solutions/sim_ac_{int(T * 1000)}ms_{Ns}sn_{motion}_co_level_{co}.bob")
            ocp.save_get_data(sol, f"solutions/sim_ac_{int(T * 1000)}ms_{Ns}sn_{motion}_co_level_{co}_tmp.bob")
