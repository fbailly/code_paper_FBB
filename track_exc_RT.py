...
if __name__ == "__main__":
    ...
    for co in range(0, nb_co_lvl):
        ...
        motion = 'REACH2'
        with open(
                f"solutions/sim_ac_8000ms_800sn_{motion}_co_level_{co}.bob", 'rb'
        ) as file:
        ...
        if use_torque:
            u_ref = np.concatenate((np.zeros((nbGT, Ns+1)), controls['muscles'][:, start_delay:]))
        else:
        ...
        for marker_lvl in marker_range:
            ...
            for EMG_lvl in emg_range:
                ...
                # Loop for simulate some tries, generate new random noise to each iter
                for tries in range(nb_try):
                    ...
                    for iter in range(1, ceil((Ns+1)/rt_ratio-Ns_mhe)):
                        ...
                        # u0 = np.concatenate((np.zeros((nbGT, Ns_mhe)), u0[nbGT:, :]))
